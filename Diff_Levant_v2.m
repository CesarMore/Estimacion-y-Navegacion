...
for i=1:length(t)-1
    ...
    phi1 = mu1*abs(x1(i)-f1(i))^(1/2)*sign(x1(i)-f1(i))+ mu2*abs(x1(i)-f1(i))^(3/2)*sign(x1(i)-f1(i));
    phi2 = (mu1^2/2)*sign(x1(i)-f1(i))+ 2*mu1*mu2*(x1(i)-f1(i))+ (3/2)*mu2^2*abs(x1(i)-f1(i))^2*sign(x1(i)-f1(i));
    
    u = (-lamda*(phi1) + x2(i));

    x1(i+1) = x1(i) + T*u;
    x2(i+1) = x2(i) + T*(-alpha*phi2);
